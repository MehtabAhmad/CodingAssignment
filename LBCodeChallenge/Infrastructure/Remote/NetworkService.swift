...
class RemoteAPI {
    ...
    func getRequest<T:Decodable>(url: String, parameters: [String : AnyObject] = [:], headers: [String : String] = [:], completion: @escaping (T) -> Void, failure: @escaping (String) -> Void ) {
    ...
    func handleResponse<T:Decodable>(response: DataResponse<Any>, completion: @escaping (T) -> Void, failure: @escaping (String) -> Void) {
        
        switch response.result {
        case .success:
            self.showRequestDetailForSuccess(responseObject: response)
            parseJSON(json: JSON(response.result.value!), completion: completion, failure: failure)
            break
            
        case .failure:
            self.showRequestDetailForFailure(responseObject: response)
            failure(response.result.error?.localizedDescription ?? "Server request failed")
            break
        }
    }
